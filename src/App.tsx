...
import { getMotionFromProgress, getPageScrollProgress } from './modelMotion'
import './App.css'
...
function HeroCakeModel() {
  ...
  useEffect(() => {
    ...
    let scrollProgress = getPageScrollProgress()
    const motionCurrent = {
      posX: 0,
      posY: 0,
      posZ: 0,
    ...
    const handleScroll = () => {
      scrollProgress = getPageScrollProgress()
    }
    ...
    const fitCameraToModel = () => {
      ...
      camera.position.set(rightAnchorX * 0.32, modelSize.y * 0.1, distance)
      targetLookAt.set(rightAnchorX, 0, 0)
    ...
    const animate = () => {
      ...
      if (model) {
        const scrollMotion = getMotionFromProgress(scrollProgress)
        ...
        motionCurrent.floatAmplitude = THREE.MathUtils.lerp(
          ...
          scrollMotion.floatAmplitude,
          0.05,
        )

        model.rotation.x = motionCurrent.rotX + mouseCurrent.x
        model.rotation.y = motionCurrent.rotY + mouseCurrent.y
...
function App() {
  return (
    <main className="page">
      ...
      <section className="gallery-section section-grid">
